...
`;class ut extends at{constructor(){super(...arguments),this._collapsed=!1}static get styles(){return ht}setConfig(t){if(!t.entity)throw new Error("Please define an entity in the card configuration");this._config={theme:"default",layout:"default",...t}}updated(t){if(super.updated(t),t.has("_config")){const t=this._config?.theme||"default";"default"===t?this.removeAttribute("theme"):this.setAttribute("theme",t),this._config?.accent_color?this.style.setProperty("--appliance-accent",this._config.accent_color):this.style.removeProperty("--appliance-accent"),this._config?.main_color?this.style.setProperty("--appliance-bg",this._config.main_color):this.style.removeProperty("--appliance-bg")}}getCardSize(){return"compact"===this._config?.layout||this._collapsed?2:5}static getConfigForm(){return{schema:[{name:"entity",required:!0,label:"Washer Entity",selector:{entity:{integration:"ifb_washer_local"}}},{name:"name",label:"Custom Title",selector:{text:{}}},{name:"theme",label:"Theme",selector:{select:{options:[{label:"Classic",value:"default"},{label:"Material You",value:"material_you"}]}}},{name:"layout",label:"Card Layout",selector:{select:{options:[{label:"Default (Full)",value:"default"},{label:"Compact",value:"compact"}]}}},{name:"",type:"expandable",title:"Theming & Colors",schema:[{name:"accent_color",label:"Accent Color Override",selector:{text:{}}},{name:"main_color",label:"Background Color Override",selector:{text:{}}}]}]}}_haptic(t="light"){window.dispatchEvent(new CustomEvent("haptic",{detail:t}))}_showToast(t){this._haptic("warning"),this.dispatchEvent(new CustomEvent("hass-notification",{bubbles:!0,composed:!0,detail:{message:t}}))}_resolveEntities(){const t=this._config.entity,e=this._config;let i=e.power_switch,s=e.start_button,o=e.pause_button,r=e.cancel_button,a=e.program_select,n=e.spin_select,c=e.temperature_select,l=e.delay_select,p=e.child_lock_switch,d=e.machine_state_sensor,h=e.time_remaining_sensor,u=e.cycle_progress_sensor,m=e.tub_temp_sensor,f=e.motor_speed_sensor,g=e.door_locked_sensor,_="";const v=this.hass?.entities;if(v&&v[t]){const e=v[t].device_id;if(e)for(const[t,b]of Object.entries(v)){if(b.device_id!==e)continue;const v=b.unique_id||"",y=b.translation_key||"";i||!v.endsWith("_power_switch")&&"power"!==y||(i=t),s||!v.endsWith("_start")&&"start"!==y||(s=t),o||!v.endsWith("_pause")&&"pause"!==y||(o=t),r||!v.endsWith("_cancel")&&"cancel"!==y||(r=t),a||!v.endsWith("_program_select")&&"program_select"!==y||(a=t),n||!v.endsWith("_spin_speed_select")&&"spin_speed_select"!==y||(n=t),c||!v.endsWith("_temperature_select")&&"temperature_select"!==y||(c=t),l||!v.endsWith("_delay_start_select")&&"delay_start_select"!==y||(l=t),p||!v.endsWith("_child_lock_switch")&&"child_lock_switch"!==y||(p=t),d||!v.endsWith("_state")&&"machine_state"!==y||(d=t),h||!v.endsWith("_time_remaining")&&"time_remaining"!==y||(h=t),u||!v.endsWith("_cycle_progress")&&"cycle_progress"!==y||(u=t),m||!v.endsWith("_tub_temperature")&&"tub_temperature"!==y||(m=t),f||!v.endsWith("_motor_rpm")&&"motor_rpm"!==y&&!v.endsWith("_motor_speed")||(f=t),g||!v.endsWith("_door_locked")&&"door_locked"!==y||(g=t),_||!v.endsWith("_problem")&&"problem"!==y||(_=t)}}const b=["_power","_power_switch","_machine_state","_running","_time_remaining","_program_duration","_cycle_progress","_start","_pause","_cancel","_program_select","_spin_speed_select","_temperature_select","_delay_start_select","_child_lock_switch","_tub_temperature","_motor_speed","_door_locked"];let y=t.split(".")[1]||"";for(const t of b)if(y.endsWith(t)){y=y.substring(0,y.length-t.length);break}return{power:i||`switch.${y}_power`,start:s||`button.${y}_start`,pause:o||`button.${y}_pause`,cancel:r||`button.${y}_cancel`,program:a||`select.${y}_program_select`,spin:n||`select.${y}_spin_speed_select`,temp:c||`select.${y}_temperature_select`,delay:l||`select.${y}_delay_start_select`,childLock:p||`switch.${y}_child_lock_switch`,state:d||`sensor.${y}_machine_state`,remaining:h||`sensor.${y}_time_remaining`,progress:u||`sensor.${y}_cycle_progress`,tubTemp:m||`sensor.${y}_tub_temperature`,rpm:f||`sensor.${y}_motor_speed`,door:g||`binary_sensor.${y}_door_locked`,problem:_||`binary_sensor.${y}_problem`}}_callService(t,e,i){this.hass&&this.hass.callService(t,e,i)}_togglePower(t,e){e?(this._haptic("medium"),this._callService("switch","toggle",{entity_id:t.power})):this._showToast("Device is offline")}_triggerButton(t,e,i){e?i?(this._haptic("light"),this._callService("button","press",{entity_id:t})):this._showToast("Turn on the washer to start cycle"):this._showToast("Device is offline")}_selectOption(t,e,i,s,o,r=!1){i?s?o&&r?this._showToast("Pause cycle to change wash program"):(this._haptic("selection"),this._callService("select","select_option",{entity_id:t,option:e})):this._showToast("Turn on the washer to adjust settings"):this._showToast("Device is offline")}_toggleChildLock(t,e,i){e?i?(this._haptic("medium"),this._callService("switch","toggle",{entity_id:t})):this._showToast("Turn on the washer to toggle child lock"):this._showToast("Device is offline")}_formatRemaining(t){if(!t||t<=0)return"00:00";const e=t%60;return`${Math.floor(t/60).toString().padStart(2,"0")}:${e.toString().padStart(2,"0")}`}render(){if(!this.hass||!this._config)return F;const t=this._resolveEntities(),e=this.hass.states[t.power],i=this.hass.states[t.state],s=this.hass.states[t.remaining],o=this.hass.states[t.progress],r=this.hass.states[t.program],a=this.hass.states[t.spin],n=this.hass.states[t.temp],c=this.hass.states[t.delay],l=this.hass.states[t.childLock],p=this.hass.states[t.tubTemp],d=this.hass.states[t.rpm],h=this.hass.states[t.door],u=this.hass.states[t.problem],m=Boolean(e&&"unavailable"!==e.state&&"unknown"!==e.state),f=m&&"on"===e.state,g=i?.state||(f?"Standby":"Off"),_=f&&Boolean(!["Standby","Idle","Complete","Paused","Off","unknown","unavailable"].includes(g)),v=f&&"Paused"===g,b=f&&"Complete"===g,y=s&&parseInt(s.state,10)||0,$=o?Math.min(100,Math.max(0,parseFloat(o.state)||0)):0,x=r?.state||"",w=a?.state||"",A=n?.state||"",k=c?.state||"No Delay",C="on"===l?.state,S="off"===h?.state||!0===h?.attributes?.door_locked,E="on"===u?.state,P=p&&parseInt(p.state,10)||0,T=d&&parseInt(d.state,10)||0,O=this._config.name||e?.attributes?.friendly_name?.replace(/ Power$/,"")||"IFB Washing Machine";let D="Off";if(m){if(E)D="Error / Attention Required";else if(b)D="Cycle Complete";else if(f){const t=[x||g];A&&"None"!==A&&t.push(A),w&&"None"!==w&&t.push(w),D=t.join(" • ")}}else D="Offline";const M="compact"===this._config.layout,R=this._collapsed,U=2*Math.PI*70,z=U-$/100*U;return W`
      <ha-card>
        <!-- Header -->
        <div class="header">
          ...
          <div class="header-right">
            ${M?F:W`
                ...
                `}
            <button
              class="power-btn ${f?"on":""} ${m?"":"disabled"}"
              title="${m?f?"Turn Off":"Turn On":"Device is offline"}"
              @click=${()=>this._togglePower(t,m)}
            >
        ...
        ${M||R?this._renderCompactBody(t,m,f,_,v,g,y,$):this._renderFullBody(t,m,f,_,v,b,g,y,$,70,U,z,x,w,A,k,C,S,E,P,T,r?.attributes?.options||[],a?.attributes?.options||[],n?.attributes?.options||[],c?.attributes?.options||[])}
    ...
    `}_renderCompactBody(t,e,i,s,o,r,a,n){return W`
    ...
    `}_renderFullBody(t,e,i,s,o,r,a,n,c,l,p,d,h,u,m,f,g,_,v,b,y,$,x,w,A){return W`
      ...
      <div class="segmented-bar">
        ${($.length>0?$:["Mix / Daily","Cotton","Express 15","Tub Clean"]).map(o=>W`
            <button
              class="segment-btn ${h===o?"active":""} ${e&&i&&!s?"":"disabled"}"
      ...
      <div class="segmented-bar">
        ${(w.length>0?w:["Cold","20°C","30°C","40°C","60°C","95°C"]).map(s=>W`
              <button
                class="segment-btn ${m===s?"active":""} ${e&&i?"":"disabled"}"
      ...
      <div class="segmented-bar">
        ${(x.length>0?x:["No Spin","400","600","800","1000","1200","1400 RPM"]).map(s=>{const o=u===s,r=s.replace(" RPM","");return W`
            <button
